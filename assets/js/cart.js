...
async function loadProducts(){
  try {
    ...
    if (els.category) buildCategoryFilter();
    state.filtered = [...state.products];
    if (els.grid) renderProducts();
  } catch (e){
    if (els.grid) els.grid.innerHTML = `<div class="col-12 text-danger small">Error cargando productos</div>`;
    console.error(e);
  }
}
...
function bindUI(){
  ...
  if (els.category) els.category.addEventListener('change', applyFilters);
  ...
  if (els.checkoutBtn) els.checkoutBtn.addEventListener('click', openPaymentModal);
  if (els.clearCartBtn) els.clearCartBtn.addEventListener('click', clearCart);
}

function applyFilters(){
  if (!els.search || !els.category) return;
...
function sendWhatsApp(){
  ...
  const phone = '56971307840'; // Cambiar
